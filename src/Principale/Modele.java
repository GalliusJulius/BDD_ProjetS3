...
public class Modele extends Observable{
	...
	public void seConnecter() throws SQLException {
		if(fenetreActu instanceof AffichageConnection) {
			...
			// VÃ©rifie que la connection est bien effectuÃ©e :
			System.out.println("ConnectÃ© : " + cnt.isValid(100));
	...
	public void mettreAjour(String immat) throws SQLException {
		...
		if(stt != null) {
			...
			//la requete ne marque pas? pourtant elle fonctionne sur data grip :( ps : le truc pour calculer le nb de jour marche !
			/*
			PreparedStatement stt2 = cnt.prepareStatement("select TARIF_JOUR, TARIF_HEBDO from TARIF"
					...
					+"where MODELE like (select MODELE from VEHICULE where NO_IMM like '1234ya54')");
			//stt.setString(1, immat);
			ResultSet res = stt2.executeQuery();
			double prixJour = res.getDouble(1);
			double prixHebdo = res.getDouble(2);
			System.out.println(prixJour +" "+prixHebdo);
			int nbJour =(int)((dateF.getTime() - dateD.getTime() )/(1000*60*60*24)); 
			double prix = (nbJour%7*prixJour + ((int)(nbJour/7))*prixHebdo);
			stt2.close();
			*/
		}
		rechercher();
		JOptionPane jop1 = new JOptionPane();
		jop1.showMessageDialog(null, "Votre réservation est confirmé, elle coutera : x €", "Tarif", JOptionPane.INFORMATION_MESSAGE);
	}
